...
Return ONLY one JSON object — no markdown fences, no prose before or after. Use EXACTLY this shape:
{
 ...
 "headline": string (one line — the single most material takeaway),
...
export default function App() {
  ...
  const runLive = async () => {
    ...
    const userContent =
      ...
      `Product name: ${liveName || "(unnamed)"}\n` +
      `Category: ${liveCategory || "(unspecified)"}\n` +
      `Target markets: ${liveMarkets.length ? liveMarkets.join(", ") : "EU, UK, US"}\n` +
      `Intended claims: ${liveClaims || "(none stated)"}\n\n` +
    ...
    try {
      ...
      timers.current.forEach(clearTimeout); timers.current = [];
      setLiveError("The live assessment couldn't be completed. Check the formulation input and run it again.");
